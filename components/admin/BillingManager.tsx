      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
            <CreditCard className="w-6 h-6 text-teal-600" /> Billing & SPP Management
          </h2>
          <p className="text-gray-500">Monitor monthly student tuition and automated payment tracking.</p>
        </div>
        <div className="flex gap-4">
           <div className="bg-green-50 border border-green-100 px-4 py-2 rounded-xl text-right">
              <div className="text-[10px] font-bold text-green-600 uppercase tracking-widest">Collected</div>
              <div className="text-xl font-black text-green-900">Rp {totalCollected.toLocaleString()}</div>
           </div>
           <div className="bg-red-50 border border-red-100 px-4 py-2 rounded-xl text-right">
              <div className="text-[10px] font-bold text-red-600 uppercase tracking-widest">Outstanding</div>
              <div className="text-xl font-black text-red-900">Rp {totalOutstanding.toLocaleString()}</div>
           </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
         <div className="relative md:col-span-2">
            ...
            <input 
              ...
              className="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-2xl text-sm shadow-sm outline-none focus:ring-1 focus:ring-teal-500"
              value={searchQuery}
              onChange={e => setSearchQuery(e.target.value)}
            />
         </div>
         <select 
            className="border border-gray-200 rounded-2xl px-4 py-3 text-sm font-bold text-gray-600 bg-white cursor-pointer hover:bg-gray-50 outline-none"
            value={monthFilter}
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-gray-100">
                ...
                <tr key={inv.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                       <div className="w-8 h-8 bg-blue-100 rounded-lg flex items-center justify-center text-blue-600 font-bold text-xs">{inv.studentName.charAt(0)}</div>
                       <div>
                          <div className="text-sm font-bold text-gray-900">{inv.studentName}</div>
                          <div className="text-[10px] text-gray-400 font-bold uppercase">{inv.month}</div>
                       </div>
                    </div>
                  </td>
                  <td className="px-6 py-4 text-sm font-black text-gray-900">Rp {inv.amount.toLocaleString()}</td>
                  <td className="px-6 py-4 text-xs font-bold text-gray-500 uppercase">{new Date(inv.dueDate).toLocaleDateString('id-ID')}</td>
                  <td className="px-6 py-4">
                     <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase flex items-center gap-1.5 w-fit ${inv.status === 'PAID' ? 'bg-green-100 text-green-700 border border-green-200' : 'bg-red-100 text-red-700 border border-red-200 animate-pulse'}`}>
                        {inv.status === 'PAID' ? <CheckCircle className="w-3 h-3" /> : <Clock className="w-3 h-3" />}
                        {inv.status}
                     </span>
                     {inv.remindedAt && inv.status === 'UNPAID' && (
                        <div className="text-[8px] text-orange-500 font-black uppercase mt-1">Last Reminded: {new Date(inv.remindedAt).toLocaleDateString()}</div>
                     )}
                  </td>
                       ...
                       {inv.status === 'UNPAID' ? (
                          <>
                             <button 
                                ...
                                className="px-4 py-2 bg-green-600 text-white text-[10px] font-black uppercase rounded-xl hover:bg-green-700 transition-all shadow-md active:scale-95"
                             >
                                Mark Paid
                             </button>
                             <button 
                                onClick={() => openWhatsAppReminder(inv)}
                                className="p-2.5 bg-green-50 text-green-600 hover:bg-green-600 hover:text-white rounded-xl border border-green-200 transition-all active:scale-90"
                          ...
                          <span className="text-[10px] text-gray-300 font-black uppercase italic">Payment Verified</span>
                       )}
                    </div>
                  </td>
                </tr>
              ...
              {filteredInvoices.length === 0 && (
                 <tr><td colSpan={5} className="px-6 py-12 text-center text-gray-400 italic">No invoices found for this criteria.</td></tr>
              )}
            </tbody>
          </table>
        </div>
