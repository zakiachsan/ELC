/**
 * Import Script for TK, SD, SMP CHARITAS BATAM
 * Usage: node scripts/import-charitas-batam.js
 */
...
const CSV_FILE = path.join(__dirname, '..', 'Daftar Student', 'TK_SD_SMP_CHARITAS_BATAM_List_Full.csv');
const OUTPUT_CSV = path.join(__dirname, '..', 'Daftar Student', 'TK_SD_SMP_CHARITAS_BATAM_List_With_Credentials.csv');
...
function determineJenjang(kelas) {
  ...
  if (k.includes('tk') || k.includes('kb')) return 'TK';
...
async function ensureLocation(name, level) {
  ...
  const { data: created, error } = await supabase.from('locations')
    .insert({ name, address: 'Batam', capacity: 1000, level }).select('id').single();
...
async function main() {
  ...
  console.log('IMPORT TK, SD & SMP CHARITAS BATAM');
  console.log('='.repeat(50) + '\n');

  // Create locations
  const tkId = await ensureLocation('TK CHARITAS BATAM', 'KINDERGARTEN');
  const sdId = await ensureLocation('SD CHARITAS BATAM', 'ELEMENTARY');
  const smpId = await ensureLocation('SMP CHARITAS BATAM', 'JUNIOR');
  const locationMap = { TK: tkId, SD: sdId, SMP: smpId };
  ...
  for (let i = 0; i < processedRows.length; i++) {
    ...
    try {
      ...
      const schoolInfo = `${s.jenjang} CHARITAS BATAM - ${s['Kelas']} (${s['Tipe Kelas'] || 'Regular'})`;
