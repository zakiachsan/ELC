/**
 * Import Script for SD & SMP SANG TIMUR KARANG TENGAH
 * Usage: node scripts/import-sang-timur-karang-tengah.js
 */
...
const CSV_FILE = path.join(__dirname, '..', 'Daftar Student', 'SD_SMP_SANG_TIMUR_KARANG_TENGAH_List_Full.csv');
const OUTPUT_CSV = path.join(__dirname, '..', 'Daftar Student', 'SD_SMP_SANG_TIMUR_KARANG_TENGAH_List_With_Credentials.csv');
...
async function ensureLocation(name, level) {
  ...
  const { data: created, error } = await supabase.from('locations')
    .insert({ name, address: 'Karang Tengah, Tangerang', capacity: 500, level }).select('id').single();
...
async function main() {
  ...
  console.log('IMPORT SD & SMP SANG TIMUR KARANG TENGAH');
  console.log('='.repeat(50) + '\n');

  // Create locations
  const sdId = await ensureLocation('SD SANG TIMUR KARANG TENGAH', 'ELEMENTARY');
  const smpId = await ensureLocation('SMP SANG TIMUR KARANG TENGAH', 'JUNIOR');
  ...
  for (let i = 0; i < processedRows.length; i++) {
    ...
    try {
      ...
      const schoolInfo = `${s['Jenjang']} SANG TIMUR KARANG TENGAH - ${s['Kelas']} (${s['Tipe Kelas'] || 'Regular'})`;
