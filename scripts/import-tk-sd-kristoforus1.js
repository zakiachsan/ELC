/**
 * Import Script for TK & SD KRISTOFORUS 1
 * Usage: node scripts/import-tk-sd-kristoforus1.js
 */
...
const CSV_FILE = path.join(__dirname, '..', 'Daftar Student', 'TK_SD_KRISTOFORUS_1_List_Full.csv');
const OUTPUT_CSV = path.join(__dirname, '..', 'Daftar Student', 'TK_SD_KRISTOFORUS_1_List_With_Credentials.csv');
...
async function ensureLocation(name, level) {
  ...
  const { data: created, error } = await supabase.from('locations')
    .insert({ name, address: 'Jakarta', capacity: 700, level }).select('id').single();
...
async function main() {
  ...
  console.log('IMPORT TK & SD KRISTOFORUS 1');
  console.log('='.repeat(50) + '\n');

  // Create locations
  const tkId = await ensureLocation('TK KRISTOFORUS 1', 'KINDERGARTEN');
  const sdId = await ensureLocation('SD KRISTOFORUS 1', 'ELEMENTARY');
  const locationMap = { TK: tkId, SD: sdId };
  ...
  for (let i = 0; i < processedRows.length; i++) {
    ...
    try {
      ...
      const schoolInfo = `${s.jenjang} KRISTOFORUS 1 - ${s['Kelas']} (${s['Tipe Kelas'] || 'Regular'})`;
