/**
 * Direct Import Script for SMP & SMA BHK
 * Usage: node scripts/import-students-smp-sma-bhk.js
 */
...
const CSV_FILE = path.join(__dirname, '..', 'Daftar Student', 'SMP_SMA_BHK_List_With_Credentials.csv');
...
async function ensureLocationsExist() {
  const locationNames = ['SMP BHK', 'SMA BHK'];
...
async function importStudents() {
  console.log('===========================================');
  console.log('IMPORT SMP & SMA BHK');
  console.log('===========================================\n');
  ...
  locations?.forEach((loc) => {
    if (loc.name === 'SMP BHK') locationMap['SMP'] = loc.id;
    if (loc.name === 'SMA BHK') locationMap['SMA'] = loc.id;
  });

  console.log('Location IDs:', locationMap);

  if (!locationMap['SMP'] || !locationMap['SMA']) {
    console.error('\nError: SMP or SMA BHK not found in locations table!');
    process.exit(1);
  }
  ...
  for (let i = 0; i < students.length; i++) {
    ...
    try {
      ...
      const schoolInfo = `${jenjang} BHK - ${s['Kelas']} (${s['Tipe Kelas']})`;
