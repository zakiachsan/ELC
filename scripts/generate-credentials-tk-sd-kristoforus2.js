/**
 * Generate Email and Password for TK_SD_KRISTOFORUS_2 students
 * Usage: node scripts/generate-credentials-tk-sd-kristoforus2.js
 *
 * Note: KB (Kelompok Bermain) is marked as "SD" but should be "TK".
 * This script corrects the Jenjang based on class name.
 */
...
const INPUT_FILE = path.join(__dirname, '..', 'Daftar Student', 'TK_SD_KRISTOFORUS_2_List_Full.csv');
const OUTPUT_FILE = path.join(__dirname, '..', 'Daftar Student', 'TK_SD_KRISTOFORUS_2_List_With_Credentials.csv');
...
function determineJenjang(kelas) {
  ...
  // KB (Kelompok Bermain) and TK classes should be TK
  if (kelasLower.includes('kb') || kelasLower.includes('tk')) {
    return 'TK';
  }

  // Extract the class number for SD
  ...
  // Default to SD if can't determine
  return 'SD';
}
...
function main() {
  console.log('===========================================');
  console.log('GENERATE CREDENTIALS - TK & SD KRISTOFORUS 2');
  console.log('===========================================\n');
