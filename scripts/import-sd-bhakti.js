/**
 * Import Script for SD BHAKTI
 * Usage: node scripts/import-sd-bhakti.js
 */
...
const CSV_FILE = path.join(__dirname, '..', 'Daftar Student', 'SD_BHAKTI_List_Full.csv');
const OUTPUT_CSV = path.join(__dirname, '..', 'Daftar Student', 'SD_BHAKTI_List_With_Credentials.csv');
...
async function ensureLocation(name, level) {
  ...
  const { data: created, error } = await supabase.from('locations')
    .insert({ name, address: 'Jakarta', capacity: 600, level }).select('id').single();
...
async function main() {
  ...
  console.log('IMPORT SD BHAKTI');
  console.log('='.repeat(50) + '\n');

  const sdId = await ensureLocation('SD BHAKTI', 'ELEMENTARY');
  ...
  for (let i = 0; i < processedRows.length; i++) {
    ...
    try {
      ...
      const schoolInfo = `SD BHAKTI - ${s['Kelas']} (${s['Tipe Kelas'] || 'Regular'})`;
