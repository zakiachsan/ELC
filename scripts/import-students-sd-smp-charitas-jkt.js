/**
 * Direct Import Script for SD & SMP CHARITAS JKT
 * Usage: node scripts/import-students-sd-smp-charitas-jkt.js
 */
...
const CSV_FILE = path.join(__dirname, '..', 'Daftar Student', 'SD_SMP_CHARITAS_JKT_List_With_Credentials.csv');
...
async function ensureLocationsExist() {
  const locationNames = ['SD CHARITAS JKT', 'SMP CHARITAS JKT'];
  ...
  for (const name of locationNames) {
    ...
    if (!existing) {
      ...
      const level = name.startsWith('SD') ? 'ELEMENTARY' : 'JUNIOR';
...
async function importStudents() {
  console.log('===========================================');
  console.log('IMPORT SD & SMP CHARITAS JKT');
  console.log('===========================================\n');
  ...
  const { data: locations, error: locError } = await supabase
    .from('locations')
    .select('id, name')
    .in('name', ['SD CHARITAS JKT', 'SMP CHARITAS JKT']);
  ...
  locations?.forEach((loc) => {
    if (loc.name === 'SD CHARITAS JKT') locationMap['SD'] = loc.id;
    if (loc.name === 'SMP CHARITAS JKT') locationMap['SMP'] = loc.id;
  });

  console.log('Location IDs:', locationMap);

  if (!locationMap['SD'] || !locationMap['SMP']) {
    console.error('\nError: SD or SMP CHARITAS JKT not found in locations table!');
    process.exit(1);
  }
  ...
  for (let i = 0; i < students.length; i++) {
    ...
    try {
      ...
      const schoolInfo = `${jenjang} CHARITAS JKT - ${s['Kelas']} (${s['Tipe Kelas']})`;
