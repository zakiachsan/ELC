/**
 * Import Script for TK & SD ST VINCENTIUS
 * Usage: node scripts/import-st-vincentius.js
 */
...
const CSV_FILE = path.join(__dirname, '..', 'Daftar Student', 'TK_SD_SMP_ST_VINCENTIUS_List_Full.csv');
const OUTPUT_CSV = path.join(__dirname, '..', 'Daftar Student', 'TK_SD_SMP_ST_VINCENTIUS_List_With_Credentials.csv');
...
async function ensureLocation(name, level) {
  ...
  const { data: created, error } = await supabase.from('locations')
    .insert({ name, address: 'Jakarta', capacity: 800, level }).select('id').single();
...
async function main() {
  ...
  console.log('IMPORT TK & SD ST VINCENTIUS');
  console.log('='.repeat(50) + '\n');

  // Create locations
  const tkId = await ensureLocation('TK ST VINCENTIUS', 'KINDERGARTEN');
  const sdId = await ensureLocation('SD ST VINCENTIUS', 'ELEMENTARY');
  ...
  for (let i = 0; i < processedRows.length; i++) {
    ...
    try {
      ...
      const schoolInfo = `${s['Jenjang']} ST VINCENTIUS - ${s['Kelas']} (${s['Tipe Kelas'] || 'Regular'})`;
