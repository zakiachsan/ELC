/**
 * Direct Import Script for TK & SD KRISTOFORUS 2
 * Usage: node scripts/import-students-tk-sd-kristoforus2.js
 */
...
const CSV_FILE = path.join(__dirname, '..', 'Daftar Student', 'TK_SD_KRISTOFORUS_2_List_With_Credentials.csv');
...
async function ensureLocationsExist() {
  const locationNames = ['TK KRISTOFORUS 2', 'SD KRISTOFORUS 2'];
  ...
  for (const name of locationNames) {
    ...
    if (!existing) {
      ...
      const level = name.startsWith('TK') ? 'KINDERGARTEN' : 'ELEMENTARY';
...
async function importStudents() {
  console.log('===========================================');
  console.log('IMPORT TK & SD KRISTOFORUS 2');
  console.log('===========================================\n');
  ...
  const { data: locations, error: locError } = await supabase
    .from('locations')
    .select('id, name')
    .in('name', ['TK KRISTOFORUS 2', 'SD KRISTOFORUS 2']);
  ...
  locations?.forEach((loc) => {
    if (loc.name === 'TK KRISTOFORUS 2') locationMap['TK'] = loc.id;
    if (loc.name === 'SD KRISTOFORUS 2') locationMap['SD'] = loc.id;
  });

  console.log('Location IDs:', locationMap);

  if (!locationMap['TK'] || !locationMap['SD']) {
    console.error('\nError: TK or SD KRISTOFORUS 2 not found in locations table!');
    process.exit(1);
  }
  ...
  for (let i = 0; i < students.length; i++) {
    ...
    try {
      ...
      const schoolInfo = `${jenjang} KRISTOFORUS 2 - ${s['Kelas']} (${s['Tipe Kelas']})`;
