/**
 * Import Script for SD SANG TIMUR CAKUNG
 * Usage: node scripts/import-sang-timur-cakung.js
 */
...
const CSV_FILE = path.join(__dirname, '..', 'Daftar Student', 'SD_SANG_TIMUR_CAKUNG_List_Full.csv');
const OUTPUT_CSV = path.join(__dirname, '..', 'Daftar Student', 'SD_SANG_TIMUR_CAKUNG_List_With_Credentials.csv');
...
async function ensureLocation(name, level) {
  ...
  const { data: created, error } = await supabase.from('locations')
    .insert({ name, address: 'Cakung, Jakarta Timur', capacity: 500, level }).select('id').single();
...
async function main() {
  ...
  console.log('IMPORT SD SANG TIMUR CAKUNG');
  console.log('='.repeat(50) + '\n');

  const sdId = await ensureLocation('SD SANG TIMUR CAKUNG', 'ELEMENTARY');
  ...
  for (let i = 0; i < processedRows.length; i++) {
    ...
    try {
      ...
      const schoolInfo = `SD SANG TIMUR CAKUNG - ${s['Kelas']} (${s['Tipe Kelas'] || 'Regular'})`;
      const { error: profileError } = await supabase.from('profiles').insert({
        id: authData.user.id, email: s.email, name: s['Nama Siswa'],
        role: 'STUDENT', status: 'ACTIVE',
        assigned_location_id: sdId, school_origin: schoolInfo
      });
