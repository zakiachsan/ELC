/**
 * Generate Email and Password for TK_SD_ABDI_SISWA_BINTARO students
 * Usage: node scripts/generate-credentials-bintaro.js
 */
...
const INPUT_FILE = path.join(__dirname, '..', 'Daftar Student', 'TK_SD_ABDI_SISWA_BINTARO_List_Full.csv');
const OUTPUT_FILE = path.join(__dirname, '..', 'Daftar Student', 'TK_SD_ABDI_SISWA_BINTARO_List_With_Credentials.csv');
...
function generateEmail(name) {
  ...
  if (parts.length === 1) {
    // Single name: use name only
    return `${parts[0].toLowerCase()}@enormous1.com`;
  }

  // Get first name and last name (last word)
...
function main() {
  console.log('===========================================');
  console.log('GENERATE CREDENTIALS - TK & SD ABDI SISWA BINTARO');
  console.log('===========================================\n');
