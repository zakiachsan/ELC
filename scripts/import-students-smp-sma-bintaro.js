/**
 * Direct Import Script for SMP & SMA ABDI SISWA BINTARO
 * Usage: node scripts/import-students-smp-sma-bintaro.js
 */
...
const CSV_FILE = path.join(__dirname, '..', 'Daftar Student', 'SMP_SMA_ABDI_SISWA_BINTARO_List_With_Credentials.csv');
...
async function ensureLocationsExist() {
  const locationNames = ['SMP ABDI SISWA BINTARO', 'SMA ABDI SISWA BINTARO'];
...
async function importStudents() {
  console.log('===========================================');
  console.log('IMPORT SMP & SMA ABDI SISWA BINTARO');
  console.log('===========================================\n');
  ...
  const { data: locations, error: locError } = await supabase
    .from('locations')
    .select('id, name')
    .in('name', ['SMP ABDI SISWA BINTARO', 'SMA ABDI SISWA BINTARO']);
  ...
  locations?.forEach((loc) => {
    if (loc.name === 'SMP ABDI SISWA BINTARO') locationMap['SMP'] = loc.id;
    if (loc.name === 'SMA ABDI SISWA BINTARO') locationMap['SMA'] = loc.id;
  });

  console.log('Location IDs:', locationMap);

  if (!locationMap['SMP'] || !locationMap['SMA']) {
    console.error('\nError: SMP or SMA ABDI SISWA BINTARO not found in locations table!');
    process.exit(1);
  }
