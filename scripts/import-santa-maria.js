/**
 * Import Script for TK, SD, SMP, SMK SANTA MARIA
 * Usage: node scripts/import-santa-maria.js
 */
...
const CSV_FILE = path.join(__dirname, '..', 'Daftar Student', 'TK_SD_SMP_SMK_SANTA_MARIA_List_Full.csv');
const OUTPUT_CSV = path.join(__dirname, '..', 'Daftar Student', 'TK_SD_SMP_SMK_SANTA_MARIA_List_With_Credentials.csv');
...
function getLevel(jenjang) {
  switch (jenjang) {
    case 'TK': return 'KINDERGARTEN';
    case 'SD': return 'ELEMENTARY';
    case 'SMP': return 'JUNIOR';
    case 'SMK': return 'SENIOR';
...
async function main() {
  ...
  console.log('IMPORT TK, SD, SMP, SMK SANTA MARIA');
  console.log('='.repeat(50) + '\n');

  // Create locations
  const tkId = await ensureLocation('TK SANTA MARIA', 'KINDERGARTEN');
  const sdId = await ensureLocation('SD SANTA MARIA', 'ELEMENTARY');
  const smpId = await ensureLocation('SMP SANTA MARIA', 'JUNIOR');
  const smkId = await ensureLocation('SMK SANTA MARIA', 'SENIOR');
  const locationMap = { TK: tkId, SD: sdId, SMP: smpId, SMK: smkId };
  ...
  // Count by jenjang
  const byJenjang = { TK: 0, SD: 0, SMP: 0, SMK: 0 };
  ...
  for (let i = 0; i < processedRows.length; i++) {
    ...
    try {
      ...
      const schoolInfo = `${s['Jenjang']} SANTA MARIA - ${s['Kelas']} (${s['Tipe Kelas'] || 'Regular'})`;
